...
function readReadmeKo(): string {
  const candidates = [
    path.join(/* turbopackIgnore: true */ process.cwd(), "..", "README.ko.md"),
    path.join(/* turbopackIgnore: true */ process.cwd(), "README.ko.md"),
  ];
  ...
  throw new Error(
    `README.ko.md not reachable. Tried: ${candidates.join(" · ")}. ` +
      `If deploying with web/ as the project root, copy README.ko.md into web/ at build time ` +
...
export const metadata: Metadata = {
  title: "README (한국어) — episteme",
  description:
    "README.md 한국어 번역본. AI 코딩 에이전트를 위한 생각의 틀 (Thinking Framework) — Sovereign Cognitive Kernel.",
};

export default function ReadmeKoPage() {
  const markdown = readReadmeKo();
  return (
    <>
      <Header />
      <main className="mx-auto max-w-3xl px-6 py-16 md:py-24" lang="ko">
        <ReactMarkdown
