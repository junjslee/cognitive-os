...
def _machine_context() -> dict[str, str]:
    ...
    return {
        ...
        "PYTHON_POLICY": "All local Python-backed agent-os commands run in Conda base at /Users/junlee/miniconda3.",
    }
...
def _managed_skills() -> list[Path]:
    selected = set(RUNTIME_MANIFEST["vendor_skills"] + RUNTIME_MANIFEST["house_skills"])
    candidates = list((REPO_ROOT / "skills" / "vendor").glob("*/SKILL.md")) + list(
        (REPO_ROOT / "skills" / "house").glob("*/SKILL.md")
    )
...
def _validate_manifest() -> int:
    ...
    all_declared = RUNTIME_MANIFEST["vendor_skills"] + RUNTIME_MANIFEST["house_skills"]

    # Forward check: declared → file exists
    for bucket in ("vendor", "house"):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("sync", help="Sync managed runtime assets into Claude, Codex, and Cursor")
