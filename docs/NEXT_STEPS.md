...
5. **GTM site + dashboard ramp (`web/`, v1 shipped — 2026-04-22, Event 18; v2 shipped — 2026-04-22, Event 19).** Landing + dashboard + three API routes live-read `$EPISTEME_HOME/framework/*.jsonl` and `$EPISTEME_PROJECT/.episteme/reasoning-surface.json` via the envelope mapper; `pnpm build` green; smoke-tested against the kernel's own on-disk state (5 real deferred-discovery records render through `/api/chain` with intact chain). v2 stages remaining:
