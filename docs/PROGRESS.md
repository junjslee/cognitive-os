...
## Event 61 — 2026-04-26 — `AGENTS.md` git workflow protocol refinement: sync-order + Path B branch-protection caveat + Footgun A/B documentation

**Scope.** Pure docs/governance artifact. ZERO new files. ZERO `kernel/*` edits. Three modified files: `AGENTS.md` (additive changes inside `## Git workflow protocol — always-clean-master`), `docs/PROGRESS.md` (this entry), `docs/NEXT_STEPS.md` (Resume-here). AGENTS.md is the cross-tool repo-operating contract (visible to Claude Code / Codex / opencode / Hermes adapters); not kernel-tier. Soak-safe by construction.

**Why it matters.** Two recurring git-workflow footguns surfaced repeatedly across Events 56-60 — both costing operator time + producing misleading error messages that read worse than the actual situation. Event-57's protocol prevents the *underlying divergence class*; Event 61 closes the *symptom-recognition gap* so future agents and the operator both stop mis-diagnosing the symptoms.

**The two footguns codified.**

1. **Footgun A · `git pull` on feature branch advances it past remote-tracking ref.** Symptom: you ran `git pull --ff-only origin master` while still on a feature branch (not master). Pull operates on the currently-checked-out branch — so the FEATURE BRANCH advances by pulling origin/master into it, not local master. Your local feature-branch HEAD is now ahead of `origin/<feature-branch>` (the remote tracking ref). Cause: `git pull --ff-only origin master` means "fast-forward CURRENT branch by pulling origin/master into it" — not "fast-forward LOCAL master." Fix: always `git checkout master` BEFORE `git pull`. Codified in Pre-Event step 3 with explicit `# FIRST` / `# THEN` annotations.
2. **Footgun B · Post-PR-merge `git branch -d` refuses with stale-upstream-tracking complaint.** Symptom: PR merged on origin, you're on master synced to origin/master, you run `git branch -d event-NN-shortname` to clean up. Git refuses with `not yet merged to refs/remotes/origin/event-NN-...-prep, even though it is merged to HEAD` — the **"even though it is merged to HEAD"** phrase is the load-bearing tell that the branch IS safely merged. Cause: PR-merge does NOT auto-update origin's source-branch ref; it stays at the last-pushed SHA. Local feature-branch's upstream tracking ref still points at that stale SHA. `git branch -d` compares against the upstream and refuses, even though the branch's tip is the merge commit's parent on master. Fix: `git branch -D event-NN-shortname` (force-delete) — safe here because `-D` only removes the local ref pointer; the underlying commit is reachable from master via the merge commit. Optionally also `git push origin --delete event-NN-shortname` to clean the stale remote ref. Distinguishing Footgun B from a real unmerged-work case: the `merged to HEAD` line in git's warning text — present = Footgun B (safe to `-D`); absent = real unmerged work, decide before deleting.

**Path B branch-protection caveat.** Path B (local fast-forward + push) requires origin to permit direct push to master. When master branch protection is enforced (recommended for any episteme deployment, currently active on `junjslee/episteme`), `git push origin master` returns `GH006: Protected branch update failed for refs/heads/master. Changes must be made through a pull request.` and Path B is impossible. The protocol now names this explicitly with `**On junjslee/episteme: Path A only.**` Operators on a fork or fresh deployment can verify their own protection state with `gh repo view --json branchProtectionRules,defaultBranchRef`.

**Files touched.** `AGENTS.md` (4 additive edits inside the existing `## Git workflow protocol — always-clean-master` section: Pre-Event step 3 order clarification + Path B caveat block + Post-Event step 13 `-d`-vs-`-D` amendment + new `### Common footguns` subsection between `### Long-term root-cause fix` and the closing `---`); `docs/PROGRESS.md` (this entry); `docs/NEXT_STEPS.md` (Resume-here Event-61 line). 3 files modified, 0 new, 0 kernel/* / hot-path touches.

**Soak status reminder.** v1.0.0 RC fresh 7-day soak active since 2026-04-23T21:23:36Z, target close ~2026-04-30. Day 2.98/7 elapsed. Day-7 grading proceeds unchanged. Episodic stream healthy (40 records / 5 day-files); deferred_discoveries growing as expected (1321 entries — raw count; `tools/sample_deferred.py --unique` dedups to actual signal); protocols.jsonl correctly empty (no Blueprint-B fence_reconstruction op has fired this soak window — expected for docs-only Events).

**Long-term root-cause-fix note unchanged.** The chkpt-hook `core/hooks/checkpoint.py` committing-to-HEAD-on-master root cause stays queued for v1.0.1+. Event 61 codifies operator-side workaround patterns; the structural fix at the hook level remains post-soak.

---

## Event 60 — 2026-04-26 — Reference anchor expansion staged (Section K): Peirce + Engelbart + Bender + Heidegger added; Girard rejected

**Scope.** Pure docs/staging artifact. ZERO new files. ZERO `kernel/*` edits. Three modified files: `docs/POST_SOAK_MIGRATION_PLAN.md` (new Section K + checklist item #6), `docs/PROGRESS.md` (this entry), `docs/NEXT_STEPS.md` (Resume-here). Soak-safe by construction — operator's explicit directive: *"Editing kernel/REFERENCES.md is a governance-class operation. We are currently in the active v1.0.0-rc1 soak window. I will NOT edit kernel/ files mid-soak."* This Event respects that — Section K is a STAGING document; execution happens post-GA-cut.

**Why it matters.** Operator surfaced four candidate reference anchors that map cleanly to current architecture (Peirce → Cognitive Arm B; Engelbart → Pillar 3 + Cognitive Arms A/C; Bender et al. → anti-Doxa ML anchor; Heidegger → anti-Doxa philosophical anchor) plus one candidate that doesn't (Girard — metaphorical stretch). Operator review pass resolved all five: ADD the four, REJECT Girard. Section K codifies the staging so post-soak execution is mechanical (copy text from § K → paste into kernel/REFERENCES.md → integrate K.3 wording into spec bodies → commit + PR-merge per Event-57 Path A).

**The four operator-resolved anchors and where each lands.**

1. **Peirce — Abductive Reasoning** anchors **Cognitive Arm B** (Causal Synthesis). The LLM-driven Reflective Session is structurally an abductive inference (Peirce 1903) — the leap from scattered deferred-discoveries to a hypothesized root cause. Critical operator insight: abduction is the *weakest* of the three inferential modes (Peirce himself named this), which is precisely why **D7** (per-entry independent verification on closure) is load-bearing. The Peirce citation makes the design's honesty about Arm B's weakest joint structurally visible. Modern philosophy-of-science companion: Lipton (Inference to the Best Explanation, 1991, optional).
2. **Engelbart — Augmenting Human Intellect / Bootstrapping** anchors **Pillar 3** + **Cognitive Arms A and C**, with **critical alignment**. Pillar 3's active-guidance loop is structurally an Engelbartian bootstrap (1962); Arms A and C extend the bootstrapping mechanism to the kernel's own knowledge artifacts. BUT episteme rejects Engelbart's 1962 optimism: IA without structural constraints produces deskilling. The kernel installs the bootstrap loop *with* the constraints Engelbart's framework lacked — D3 (re-elicitation never auto-edits), D11 (operator fatigue guardrails), D2 (retrospective-only computation). The citation positions episteme as IA-aligned-with-constraints, not naive-IA.
3. **Bender et al. — Stochastic Parrots (FAccT 2021)** anchors the **anti-Doxa posture's ML-empirical face**. LLMs predict the statistically-central next token, not context-fit truth. The Reasoning Surface's structural validation operates as the forcing function against the parrot's default behavior; failure-mode 10 (`framework-as-Doxa`) names the recursive risk that the kernel's accumulated framework itself becomes the parrot, with Cognitive Arm A (Protocol Decay) as the structural counter.
4. **Heidegger — Das Man (Being and Time, 1927)** anchors the **anti-Doxa posture's philosophical-ontological face**. Das Man — the "anonymous They" — is the existential-ontological structure by which individual accountability dissolves into the average. LLM majority-voting is Das Man at scale. The Core Question field is Heidegger's *resoluteness* operationalized as a file-system contract: the agent must declare the specific question this action answers, breaking from the averaging mode. Bender + Heidegger are deliberately a pair — Bender names what the substrate does; Heidegger names *why that matters* for the operator's authentic existence as a thinker.

**Girard — explicitly rejected (audit trail per Pillar 2 ethos).** Operator considered and rejected Girard's mimetic theory (1961) as an anti-Doxa anchor. Three reasons recorded in § K.4: (a) mimetic stretch — Girard's mimesis is interpersonal desire transmission, not statistical token prediction; the two share the surface word but differ in cognitive structure; (b) tighter alternatives chosen — Bender (ML-empirical) and Heidegger (philosophical-ontological) directly anchor the depths Girard would only metaphorically gesture at; (c) reference-quality consistency — episteme's existing references are all *direct* anchors to specific kernel mechanisms; adding Girard as the only metaphorical citation would lower the average citation precision. The rejection is final for v1.1 prep; Girard can be reconsidered if a future Event surfaces a tighter operationalization to a specific kernel mechanism (e.g., a new failure mode genuinely involving interpersonal desire transmission).

**Section K structure (staging document).** K.1 pre-flight verification (audit existing REFERENCES for citation overlap, audit FAILURE_MODES mode 10, audit DESIGN_V1_1 integration sites) · K.2 four anchor entries with exact `kernel/REFERENCES.md` insertion text in copy-paste-ready markdown blocks · K.3 kernel-wording integration snippets per Arm/Pillar (K.3.A Cognitive Arm B Goodhart resistance · K.3.B Pillar 3 introduction · K.3.C Cognitive Arms A + C introductions · K.3.D anti-Doxa preamble · K.3.E optional FAILURE_MODES mode 10 citation pointer — operator decision at execution) · K.4 Girard rejection audit trail · K.5 commit + ship via Event-57 Path A · K.6 post-execution verification (5 grep checks). The Operator decision checklist at the migration plan's bottom gains item #6 documenting this resolution.

**Files touched.** 3 modified docs files: `docs/POST_SOAK_MIGRATION_PLAN.md` (Section K added between § J and Operator decision checklist; checklist item #6 added), `docs/PROGRESS.md` (this entry), `docs/NEXT_STEPS.md` (Resume-here Event-60 line). ZERO new files. ZERO kernel/* / core/hooks/* / core/blueprints/* / src/episteme/* / tests/* / LICENSE / DESIGN_V1_0 / DESIGN_V1_1 touches. Soak-invariant verified clean before commit.

**Operator gate.** Migration plan's Operator decision checklist item #6 (Reference anchor expansion) is now ✅ RESOLVED. Five other items still pending (#1 Day-7 routing, #3 history scrub, #4 sibling-layout, #5 path-coupling audit; #2 license path resolved Event 59) — resolved at execution time post-soak. § K may execute in the same consolidated post-soak governance Event as Sections C-D-E + § J, OR as a separate follow-on Event — operator preference at execution time.

**Soak status reminder.** v1.0.0 RC fresh 7-day soak active since 2026-04-23T21:23:36Z, target close ~2026-04-30. Day-7 grading proceeds unchanged. Section K execution (the actual `kernel/REFERENCES.md` edit + spec-wording integration + CHANGELOG MAJOR/MINOR bump) executes post-GA-cut only — never during soak.
