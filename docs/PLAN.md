...
**Forward pointer (v1.1+).** Architectural vision for the post-v1.0-GA cycle is drafted at `docs/DESIGN_V1_1_REASONING_ENGINE.md` (Event 55, status `drafted (vision)`, awaiting operator review). It introduces three new cognitive arms that extend the v1.0 pillars without retracting them: Temporal Integrity (Protocol Decay), Causal Synthesis (Causal Clustering & Macro-CP Closure), Self-Consistency Convergence. v1.1 CP work begins only after the v1.1 spec status flips to `approved` AND the v1.0 GA cut completes — current cycle remains the v1.0.0 RC soak.
